...
void StreamHandler::stream_close(VideoState *is) {
    ...
    if (is->vis_texture) {
		...
		SdlRenderer::texture_destroy(is->vis_texture);
	}
	
    if (is->vid_texture) {
        //SDL_DestroyTexture(is->vid_texture);
		SdlRenderer::texture_destroy(is->vid_texture);
	}
	
    if (is->sub_texture) {
        //SDL_DestroyTexture(is->sub_texture);
		SdlRenderer::texture_destroy(is->sub_texture);
	}
...
void StreamHandler::read_thread(void *arg) {
#ifdef PROFILING_SH
	...
	NYMPH_LOG_INFORMATION("Read: HEAP Free Size: " + Poco::NumberFormatter::format(free_size) 
	...
	free_size = (uint32_t) heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
	free_block = (uint32_t) heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
	NYMPH_LOG_INFORMATION("Opened: HEAP Free Size: " + Poco::NumberFormatter::format(free_size) 
							+ ", Free Block: " + Poco::NumberFormatter::format(free_block));
	// ---
#endif
	
	// Log stream info.
	// When compiled for a small configuration (CONFIG_SMALL), long_name is 0x0.
	...
	free_size = (uint32_t) heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
	free_block = (uint32_t) heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
	NYMPH_LOG_INFORMATION("Find Info: HEAP Free Size: " + Poco::NumberFormatter::format(free_size) 
							+ ", Free Block: " + Poco::NumberFormatter::format(free_block));
	// ---
#endif

    if (find_stream_info) {
		// FIXME: commenting out stream info opts because segfault. Not needed right now.
	...
	free_size = (uint32_t) heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
	free_block = (uint32_t) heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
	NYMPH_LOG_INFORMATION("Open Stream: HEAP Free Size: " + Poco::NumberFormatter::format(free_size) 
							+ ", Free Block: " + Poco::NumberFormatter::format(free_block));
	// ---
#endif
...
#endif
        if (is->seek_req) {
			av_log(NULL, AV_LOG_INFO, "Seek request: %" PRId64 ", target: %" PRId64 "\n", is->seek_rel, is->seek_pos);
...
//      of the seek_pos/seek_rel variables
        ...
        if (infinite_buffer<1 &&
			...
			is->continue_read_thread->tryWait(wait_mutex, 10);
...
VideoState* StreamHandler::stream_open(const char *filename, AVInputFormat *iformat, 	
	...
	is->continue_read_thread = new Poco::Condition;
	...
	is->read_tid = new Poco::Thread("read_thread");
#ifdef ESP_PLATFORM
	//is->read_tid->setStackSize(12288); // 12 kB.
	//is->read_tid->setStackSize(20480); // 20 kB.
	//is->read_tid->setStackSize(22528);	// 22 kB.
	//is->read_tid->setStackSize(24576);	// 24 kB.
	//is->read_tid->setStackSize(102400);	// 100 kB.
