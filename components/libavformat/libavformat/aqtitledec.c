...
{
    int frame;
    const char *ptr = (const char*) p->buf;
...
static int aqt_read_header(AVFormatContext *s)
{
    ...
    while (!avio_feof(s->pb)) {
        ...
        if (sscanf(line, "-->> %"SCNd64, &frame) == 1) {
            ...
            if (!new_event) {
                sub = ff_subtitles_queue_insert(&aqt->q, (const uint8_t*) "\n", 1, 1);
                if (!sub)
                    return AVERROR(ENOMEM);
            }
            sub = ff_subtitles_queue_insert(&aqt->q, (const uint8_t*) line, strlen(line), !new_event);
            if (!sub)
                return AVERROR(ENOMEM);
            if (new_event) {
                sub->pts = frame;
