...
{
public:
	PooledThread(const std::string& name, int stackSize = POCO_THREAD_STACK_SIZE, bool extRAM = false);
	~PooledThread();
...
PooledThread::PooledThread(const std::string& name, int stackSize, bool extRAM): 
	_idle(true), 
	_idleTime(0), 
	_pTarget(0), 
	_name(name), 
...
PooledThread* ThreadPool::createThread()
{
	...
	name << _name << "[#" << ++_serial << "]";
	return new PooledThread(name.str(), _stackSize, _extRAM);
}
