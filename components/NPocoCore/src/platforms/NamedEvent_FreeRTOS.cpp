/*
	NamedEvent_FreeRTOS.cpp
*/


#include "NamedEvent_FreeRTOS.h"
#include "../Format.h"
...
/* #if defined(sun) || defined(__APPLE__) || defined(__osf__) || defined(__QNX__) || defined(_AIX)
	if (sem_post(_sem) != 0) { return false; }
	...
	op.sem_op  = 1;
	op.sem_flg = 0;
	if (semop(_semid, &op, 1) != 0) { return false; }
	   	//throw SystemException("cannot set named event", _name);
#endif */
	return false;
}
...
/* #if defined(sun) || defined(__APPLE__) || defined(__osf__) || defined(__QNX__) || defined(_AIX)
	...
	if (err) { return false; } // throw SystemException("cannot wait for named event", _name);
#else
	struct sembuf op;
	op.sem_num = 0;
	op.sem_op  = -1;
