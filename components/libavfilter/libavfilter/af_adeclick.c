                            ...
                            double *vector, int n, double *out)
{
    ...
    av_fast_malloc(&c->y, (unsigned int*) &c->y_size, n * sizeof(*c->y));
    y = c->y;
    if (!y)
        return AVERROR(ENOMEM);
                         ...
                         double *auxiliary, double *interpolated)
{
    ...
    av_fast_malloc(&c->matrix, (unsigned int*) &c->matrix_size, nb_errors * nb_errors * sizeof(*c->matrix));
    matrix = c->matrix;
    if (!matrix)
        return AVERROR(ENOMEM);

    av_fast_malloc(&c->vector, (unsigned int*) &c->vector_size, nb_errors * sizeof(*c->vector));
    vector = c->vector;
    if (!vector)
        return AVERROR(ENOMEM);
                        ...
                        const double *src, double *dst)
{
    ...
    av_fast_malloc(&c->histogram, (unsigned int*) &c->histogram_size, s->nb_hbins * sizeof(*c->histogram));
    if (!c->histogram)
        return AVERROR(ENOMEM);
