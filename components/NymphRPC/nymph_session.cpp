...
void NymphSession::run() {
	...
	NYMPH_LOG_DEBUG("Stack free: " + NumberFormatter::format((uint32_t) uxHighWaterMark) + " bytes.");
#endif
	...
	while (NymphServer::running) {
		if (socket.poll(timeout, Net::Socket::SELECT_READ)) {
			...
			free_size = (uint32_t) heap_caps_get_free_size(MALLOC_CAP_8BIT);
			free_block = (uint32_t) heap_caps_get_largest_free_block(MALLOC_CAP_8BIT);
			NYMPH_LOG_INFORMATION("Full HEAP Free Size: " + Poco::NumberFormatter::format(free_size) 
